...
**Last register-wide verification:** 2026-09-07  
**Current completed-item count:** 2
...
| ELT-CONTENT-006 | DONE | Remove the PO Box address from public display wherever feasible | Developer/content editor |
...
| ELT-LAUNCH-007 | WAITING ON CLIENT | Domain selected; GoDaddy purchase reported/assumed, with ownership/renewal/DNS/HTTPS evidence still open | Client/project owner |
...
| ELT-LAUNCH-013 | IN PROGRESS | Configure the scheduled time-boundary refresh | Developer/deployment owner |
...
### ELT-CONTENT-006 — Remove the PO Box address from public display

- **Description:** Remove the displayed mailing address `PO Box 813, Avila Beach, CA 93424` from as many public-facing locations as feasible, including visible page content and machine-readable structured data, while retaining it only in genuinely necessary private operational records.
- **Status:** `DONE`
- **Owner/responsible party:** Developer/content editor; removal was directly authorized in the task that executed it (2026-09-07).
- **Dependencies/client inputs:** None outstanding — no other public-facing location was found to require a postal address, and the public email/contact forms remain the working contact path.
- **Acceptance criteria:** ✅ The exact PO Box/city/state/postal-code combination is absent from all public page UI, footers, forms, metadata, JSON-LD/structured data, and generated production output. ✅ Reusable address configuration/helpers (`siteConfig.contact.mailingAddress`, `formatMailingAddressLines()`) were removed entirely, along with every import and rendering branch that used them (`Footer.astro`, `ContactForms.astro`, `AboutPageJsonLd.astro`). ✅ Repository and generated-output searches (source `.ts`/`.astro` and built `web/dist/`) find zero unintended public occurrences — the only remaining source hits are two developer comments in `siteConfig.ts`/`AboutPageJsonLd.astro` explaining the removal, not rendered content. ✅ Privacy Policy and Terms were reviewed and never referenced a mailing address (no change needed there). ✅ The public email contact path (`mailto:` link, Formspree forms) is untouched and still renders. ✅ No partial/malformed structured-data address is left — the `address` key was removed from the JSON-LD object entirely, not left with empty/partial fields. ✅ `astro check` and `astro build` both pass clean after removal. The confirmed address itself was preserved as a private record in `docs/client-questions.md` (not deleted), per this item's own "do not delete a private operational record" instruction.
- **Date added:** 2026-09-07
- **Last verified date:** 2026-09-07 — removal implemented and verified in the same session (see completion evidence).
- **Completion evidence:** Files changed: `web/src/data/siteConfig.ts` (removed `mailingAddress` field/type/initial value and the `formatMailingAddressLines` helper), `web/src/components/Footer.astro` (removed import, computed value, render branch, and `.footer-address` CSS), `web/src/components/ContactForms.astro` (same, plus `.contact-forms__mailing-address` CSS), `web/src/components/AboutPageJsonLd.astro` (removed the `address`/`PostalAddress` block and updated its doc comment). Verification: `grep -rniE "PO Box 813|Avila Beach|93424|mailingAddress|postOfficeBoxNumber|formatMailingAddressLines"` across `web/src/**/*.{ts,astro}` → only the two explanatory comments noted above; the same search against `web/dist/**` (a full rebuild with `PUBLIC_FORMSPREE_*` vars set) → zero occurrences (one initial false-positive hit on `dist/about/index.html` was a Sanity document-id UUID fragment containing "813", confirmed unrelated by inspecting the surrounding text). `npx astro check` → 0 errors/warnings/hints. `npm run build` → success. Privacy Policy/Terms reviewed via `grep -n -i "mailing\|postal\|po box\|address"` → no mailing-address content in either file before or after. Private record preserved at `docs/client-questions.md` (updated to note the public-display reversal without deleting the confirmed address).
- **Relevant files/services:** `web/src/data/siteConfig.ts`, `web/src/components/ContactForms.astro`, `web/src/components/Footer.astro`, `web/src/components/AboutPageJsonLd.astro`, generated production HTML/JSON-LD, `web/src/pages/privacy.astro`, `web/src/pages/terms.astro`, `docs/client-questions.md`.

**Recovery note (2026-09-07):** This entry was momentarily lost when an unrelated "Test change" / "undo test change" commit pair deleted the entire root `DEFERRED-WORK.md`. Recovered verbatim via `git diff 90c3c01 ed12e53 -- DEFERRED-WORK.md` (the commit that originally added it, against its parent) — this is the exact original text, not a reconstruction.
...
- **Date added:** 2026-09-03
- **Last verified date:** 2026-09-07 — the developer reports that Hunter was invited to the repository so he can connect it to his Netlify account. Invitation acceptance, repository ownership, verified-email status, exact permission, Netlify Git integration access, plan dependency, and a production integration test have not yet been evidenced, so this remains `WAITING ON CLIENT`.
...
- **Date added:** 2026-09-03
- **Last verified date:** 2026-09-07 — the developer reports that Hunter owns the intended Netlify account. Verified-email status, Hunter-owned team/site creation, plan/billing conclusion, collaborator roles, GitHub connection, configuration access, and a successful production deployment are not yet evidenced, so this remains `WAITING ON CLIENT`.
...
- **Status:** `WAITING ON CLIENT` — name selected and GoDaddy purchase is reported/assumed for planning; ownership evidence, billing/renewal/recovery, DNS, and HTTPS remain unresolved.
...
- **Date added:** 2026-09-03
- **Last verified date:** 2026-09-07 — **the client selected `theelectriclavendertrain.com`**, and the developer instructed this task to assume it has been purchased through GoDaddy. `siteConfig.siteUrl` is already `https://theelectriclavendertrain.com`. The stated assumption is enough for deployment planning but is not concrete evidence of the registrant/owner, purchase status, billing, auto-renewal, recovery access, DNS configuration, or HTTPS provisioning. Kept `WAITING ON CLIENT` rather than `DONE` because the remaining acceptance criteria are not satisfied.
...
### ELT-LAUNCH-013 — Scheduled time-boundary refresh

- **Description:** Run a daily, Pacific-time-aware check and trigger a production rebuild only when an event has crossed the static site's upcoming/past boundary, so time-based event classification updates without spending Netlify deploy credits on unchanged days.
- **Status:** `IN PROGRESS`
- **Owner/responsible party:** Developer/deployment owner.
- **Dependencies/client inputs:** ELT-LAUNCH-001, stable production build, Hunter-owned Netlify build hook, GitHub Actions secret `NETLIFY_BUILD_HOOK_URL`, GitHub repository variable `SANITY_DATASET`, schedule ownership, and alerting. The variable must be `development` during the review phase and changed to `production` at content cutover.
- **Acceptance criteria:** A daily schedule checks the correct Sanity dataset independently of the Sanity publish webhook; it triggers the correct production build only when a relevant event crosses the upcoming/past boundary (or when manually forced); timing is Pacific-time-aware; a forced test run succeeds against Hunter's Netlify site; an unchanged-day run proves that no Netlify deploy is triggered; missed/failed schedules are observable; ownership and disable/change procedures are documented.
- **Date added:** 2026-09-03
- **Last verified date:** 2026-09-07 — implementation added as a GitHub Actions workflow scheduled for 2:17 AM `America/Los_Angeles`; external hook/variable setup and live Netlify verification remain open.
- **Completion evidence:** Not complete. Repository evidence: `.github/workflows/refresh-time-based-sanity-content.yml` and `.github/scripts/trigger-netlify-on-event-transition.mjs`. Still required before `DONE`: configure the Hunter-owned hook and repository variable/secret, merge the workflow to the default branch, run a forced test, record the Netlify deploy ID, verify an unchanged-day no-deploy result, confirm failure notifications/owner, and document disable/change ownership.
- **Relevant files/services:** GitHub Actions, Hunter-owned Netlify build hook, Sanity datasets, `.github/workflows/refresh-time-based-sanity-content.yml`, `.github/scripts/trigger-netlify-on-event-transition.mjs`, `docs/production-runbook.md`, `docs/developer-guide.md`.
...
| ELT-CONTENT-006 | 2026-09-07 | Removed the publicly displayed PO Box mailing address (config, renderers, structured data) from Footer, Contact & Booking page, and About-page JSON-LD; preserved the confirmed address as a private record in `docs/client-questions.md`. | Source and generated-output (`web/dist/`) searches found zero unintended public occurrences; `astro check`/`astro build` passed clean — see ELT-CONTENT-006's own entry for full evidence. | Automated (agent session), directly authorized by the task that requested it — not yet reviewed by a human. |
...
| 2026-09-07 (sometime before this row) | `ELT-CONTENT-006` was added (mailing-address public-display removal), and the register's top-of-file "Last register-wide verification" date was bumped to 2026-09-07, in a session not fully reconstructable from this row alone — the commit that did this (`90c3c01`, "chore: add conditional Sanity-to-Netlify refresh workflow") predates `ELT-CONTENT-006`'s actual addition (found in the next commit, `ed12e53`, "Test change"). Recorded here for completeness since it could not be verified in detail; no register content from that gap other than `ELT-CONTENT-006` itself was found to differ. | Unknown/unverified beyond `ELT-CONTENT-006` | `git log`/`git diff` inspection between commits `90c3c01` and `ed12e53`. |
| 2026-09-07 | **Register incident and recovery.** An unrelated "Test change" commit (`ed12e53`) added `ELT-CONTENT-006` to the root `DEFERRED-WORK.md`; a later "undo test change" commit (`31ff9a3`), evidently intending only to revert an unrelated `web/src/data/homeData.ts` edit from the same "Test change" commit, instead deleted the entire 647+-line root `DEFERRED-WORK.md` file. A gitignored duplicate at `docs/DEFERRED-WORK.md` (untracked, no git history of its own) was found to hold the register's full content as of commit `90c3c01` — the last good state immediately before `ELT-CONTENT-006` was added. Restored the root `DEFERRED-WORK.md` from that duplicate, then recovered `ELT-CONTENT-006`'s exact original text (not a reconstruction) via `git diff 90c3c01 ed12e53 -- DEFERRED-WORK.md`, the commit that originally added it. Deleted `docs/DEFERRED-WORK.md` afterward so exactly one canonical, git-tracked copy exists at the repository root going forward, per this file's own required workflow. No item content was altered beyond re-adding what had been lost; no item was marked `DONE` as part of this recovery. | `ELT-CONTENT-006` (restored, verbatim) | `git show --stat` on `ed12e53`/`31ff9a3`; `diff` between `docs/DEFERRED-WORK.md` and `git show 90c3c01:DEFERRED-WORK.md` (byte-identical); `git diff 90c3c01 ed12e53 -- DEFERRED-WORK.md` (recovered `ELT-CONTENT-006` verbatim). |
| 2026-09-07 | Configured the four Formspree endpoints in `web/.env` (local dev only; `.env.example` and Netlify's own environment variables still need the same four values entered separately). Simplified Studio wording across the `homepage`, `aboutPage`, `musicPage`/`musicRelease`, and `galleryPage` singleton schemas per explicit per-field instructions — no field names, GROQ contracts, or validation logic changed. Corrected the "separate new-show-alert subscription" wording (never existed publicly; contradicted Hunter's one-weekly-newsletter decision) across `homepage.ts`/`contactPage.ts` schema initial values, `homeData.ts`/`contactData.ts` fallbacks, and `privacy.astro`'s two runtime mentions; patched the equivalent live `development` Sanity documents (`homepage.newsletter.body`, `contactPage.newsletterCta.body`) via the authenticated Sanity client, read back and verified, `production` untouched. Completed `ELT-CONTENT-006` (see its own entry for full evidence) — removed the publicly displayed PO Box from `siteConfig.ts`, `Footer.astro`, `ContactForms.astro`, and `AboutPageJsonLd.astro`'s structured data; the confirmed address itself was preserved as a private record in `docs/client-questions.md`, not deleted. No other register item changed; `ELT-STUDIO-001`/`ELT-STUDIO-002` remain `BACKLOG` (this pass's wording simplification was explicitly scoped, not the broader client-facing rewrite those items track). | ELT-CONTENT-006 (DONE) | `astro check`/`astro build` clean; generated-output (`web/dist/`) and source `grep` searches for new-show-alert wording and PO Box variants found zero remaining occurrences (one false-positive UUID-fragment match inspected and ruled out); Formspree form-action/`data-delivery-configured` inspection in built `contact-booking/index.html` confirmed correct per-inquiry-type endpoint mapping with none disabled; Sanity read-back queries — see each affected item's own entry. |
